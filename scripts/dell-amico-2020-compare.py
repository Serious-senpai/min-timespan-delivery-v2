            ...
            for (dirpath, _, filenames) in directory.walk():
                ...
                for filename in filenames:
                    ...
                    segments = [
                        ...
                        str(compare.fast),
                        str(compare.rrls),
                        wrap(f"=ROUND(100 * (U{row} - T{row}) / ABS(U{row}), 2)"),
                        wrap(f"=ROUND(100 * (V{row} - T{row}) / ABS(V{row}), 2)"),
                        "",
                        "",
                        ...
                        str(data["elapsed"]),
                        wrap(config["extra"]),
                        wrap(f"=ROUND(100 * (Y{row} - AK{row}) / ABS(Y{row}), 2)"),
                        wrap(f"=ROUND(100 * (Z{row} - AK{row}) / ABS(Z{row}), 2)"),
