...
if __name__ == "__main__":
    ...
    with output_csv.open("w", encoding="utf-8") as csv:
        ...
        headers = [
            ...
            "Adaptive dynamic iterations",
            "Fixed adaptive segments",
            "Fixed non-improved adaptive segments",
            "Tabu size",
            "Reset after",
            "Max elite set size",
            "Penalty exponent",
            ...
            for (dirpath, _, filenames) in directory.walk():
                ...
                for filename in filenames:
                    ...
                    segments = [
                        ...
                        str(data["adaptive_segments"]),
                        str(config["adaptive_iterations"]),
                        str(data["actual_adaptive_iterations"]),
                        str(int(config["adaptive_dynamic_iterations"])),
                        str(config["fix_adaptive_segments"]),
                        str(int(config["fix_nonimp_segments"])),
                        str(data["tabu_size"]),
                        ...
                        wrap(f"=ROUND(100 * (AB{row} - AA{row}) / ABS(AB{row}), 2)"),
                        ...
                        str(data["elapsed"]),
                        wrap(config["extra"]),
                        wrap(f"=ROUND(100 * (AD{row} - AP{row}) / ABS(AD{row}), 2)"),
                    ...
                    cursor.execute(
                        ...
                        (
                            ...
                            data["adaptive_segments"],
                            config["adaptive_iterations"],
                            data["actual_adaptive_iterations"],
                            int(config["adaptive_dynamic_iterations"]),
                            config["fix_adaptive_segments"],
                            int(config["fix_nonimp_segments"]),
                            data["tabu_size"],
                            data["reset_after"],
